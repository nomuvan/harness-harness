...
**Step 3-F: Codex側生成**（必須。Claude+Codex両対応が標準）

mapping/claude-to-codex.mdの変換ルールに従い、以下を生成:
- AGENTS.md: CLAUDE.mdからの変換。Codex固有セクション追加
- .codex/config.toml: プロファイル4種（safe/author/review/ci）、shell_environment_policy
- .agents/skills/: .claude/skills/のコピー（ディレクトリ名変更のみ。SKILL.mdは共通フォーマット）

変換時の注意:
- @importはCodexに存在しない→インライン展開
- .claude/rules/のパススコープ→AGENTS.md内に条件付き指示として記述
- permissions.allow→approval_policy+sandbox_modeに変換
- Hooks→共通3イベント（SessionStart/Stop/UserPromptSubmit）のみ変換、Claude固有イベントはAGENTS.md手順で代替
