...
**bootstrap自体もworktree+PRフローに載せる**（ユーザーから特に指示がない場合）:

1. Phase 1でリポジトリ作成後、`feature/bootstrap`ブランチ+worktreeを作成
2. Phase 2-5の全作業はworktree内で実行
3. docs/handoff.md 生成（作業計画、harness-harnessフィードバック体制）
4. docs/harness-sources.md 生成（どの知見がどこから来たか。透過性の実装）
5. commit → push → PR作成（bootstrap PR）
6. Codexレビュー完了後にマージ → worktree削除
7. harness-harness private/registry/にプロジェクト登録
8. ユーザーへのサマリー報告

これにより、対象プロジェクトの最初の変更からPRレビュー資産が残る。
...
- **ユーザーの指示を実現するために必要な情報にアクセスできない場合（URL認証必要、権限不足等）は、勝手にスキップせずユーザーにヒアリングする。** 自律完遂は指示の実現が前提。「間接情報で十分」と勝手に判断しない
- **WebFetchで取得できないSPA/JSレンダリングページ（ChatGPT共有リンク、X投稿等）はPlaywright MCPのbrowser_evaluateで取得を試みる。** 詳細はkb/techniques/playwright-mcp-web-scraping.md参照
