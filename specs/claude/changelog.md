...
- Bedrock セットアップウィザードをログイン画面から直接起動可能に
- サブスクリプションユーザー向け `/cost` でモデル別・キャッシュヒット別コスト内訳表示
- `/release-notes` がバージョン選択インタラクティブピッカーに変更
- Remote Control セッション名のデフォルトプレフィックスがホスト名に
- Pro ユーザーにプロンプトキャッシュ有効期限ヒントをフッター表示
- `/tag` コマンド削除
- `/vim` コマンド削除（`/config` → Editor mode に統合）
- Write ツールのdiff計算が大規模ファイルで60%高速化
- サブエージェント起動時の "Could not determine pane count" エラー修正
- prompt-type Stop フックの誤動作修正
- ツール入力の配列/オブジェクトJSON文字列バリデーション修正
- 拡張思考のホワイトスペースのみテキストブロックでAPI 400エラー修正
- フィードバックサーベイの誤送信修正
