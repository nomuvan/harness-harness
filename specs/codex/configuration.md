...
| レベル | パス | 用途 |
|--------|------|------|
| グローバル | `~/.codex/skills/` | 全プロジェクト共通のスキル |
| プロジェクト | `.codex/skills/` | プロジェクト固有のスキル |
...
Codex CLI は Hooks システムを実験的にサポートする。Claude Code の Hooks と比べると、対応イベントとハンドラ種別が限定されている。

### 7.1 機能フラグによる有効化

Hooks はデフォルト無効。以下のいずれかで有効化する:
...
> **注意**: `codex_hooks` は開発中（under development）のフラグであり、デフォルトは `false`。

### 7.2 設定形式

`config.toml` に `[[hooks]]` テーブル配列で定義する:
