...
### 3. Hooks はネイティブ + ラッパースクリプトの併用で埋める

> **2026-03-24 更新**: Codex CLI が Hooks を実験的サポート開始（`codex_hooks` フラグ、デフォルト無効）。`SessionStart`, `Stop`, `UserPromptSubmit` の 3 イベント・command ハンドラのみ対応。

- Codex の Hooks で対応可能な範囲:
  - `SessionStart` → preflight 処理（仕様書・テンプレート・registry の整合確認）
  - `Stop` → postflight 処理（出力保存、ログ整形、要約抽出）
  - `UserPromptSubmit` → プロンプトのバリデーション（終了コード 2 でブロック可能）
- Codex の Hooks で対応できない範囲は引き続き `scripts/` 側に持つ:
  - ツール実行前後のバリデーション（Claude の `PreToolUse` / `PostToolUse` 相当）
  - HTTP / Prompt / Agent ハンドラが必要な処理
  - invoke: `codex exec` の統一実行
  - validate: 生成されたハーネスの構造チェック
- `codex_hooks` が安定版になり次第、ラッパースクリプトからネイティブ Hooks への段階的移行を計画する
...
- Hooks を使った決定論的ガードレール（17+ イベント。Codex は 3 イベントのみ実験的サポート）
- Skills によるオンデマンド知識注入（Codex も stable サポート開始。SKILL.md 共通）
...
- `--yolo` を使う通常フロー
- symlink ベース配布
- Claude の Hooks の全イベントを Codex に無理に擬似再現すること（共通 3 イベントはネイティブ活用する）
